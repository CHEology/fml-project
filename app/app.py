...
def academic_cv_signals(text: str) -> dict[str, Any]:
    ...
    publication_count = len(
        re.findall(
            r"\b(?:journal articles?|preprints?|physical review|nature|science|cell|"
            r"proceedings|conference|workshop|symposium|arxiv|doi|ieee|acm|springer|"
            r"published in|presented at|journal of)\b",
            lowered,
        )
    )
...
def extract_work_history(text: str) -> dict[str, Any]:
    ...
    for idx, line in enumerate(lines):
        ...
        for match in DATE_RANGE_REGEX.finditer(line):
            ...
            # Faculty and research staff are NOT academic in the student sense —
            # they are full-time professional roles and should count as FT months.
            is_faculty = any(
                token in context
                for token in (
                    "professor",
            ...
            is_academic = not is_faculty and any(
                token in context
                for token in (
                    "teaching assistant",
                    "research assistant",
            ...
            for kw, label in (
                ...
                ("principal investigator", "Lead / Executive"),
                ("full professor", "Lead / Executive"),
                ("professor", "Senior"),
                ("associate professor", "Senior"),
                ("assistant professor", "Mid"),
                ("postdoctoral", "Mid"),
                ("postdoc", "Mid"),
...
def detect_profile(
    ...
    # Direct faculty title check — professors should never be Entry regardless
    # of whether employment dates were parsed correctly.
    lowered_resume = resume_text.lower()
    is_full_professor = any(
        kw in lowered_resume
        for kw in ("full professor", " professor ", "\nprofessor", "professor of")
    )
    is_assoc_professor = "associate professor" in lowered_resume
    is_asst_professor = "assistant professor" in lowered_resume
    is_postdoc = any(kw in lowered_resume for kw in ("postdoctoral", "postdoc"))
    is_pi = any(
        kw in lowered_resume
        for kw in ("principal investigator", " pi,", " pi\n", "(pi)")
    )

    faculty_floor = None
    if is_full_professor or is_pi:
        faculty_floor = "Lead / Executive"
    elif is_assoc_professor:
        faculty_floor = "Senior"
    elif is_asst_professor:
        faculty_floor = "Mid"
    elif is_postdoc:
        faculty_floor = "Mid"

    if faculty_floor and progression_order.index(
        faculty_floor
    ) > progression_order.index(seniority):
        seniority = faculty_floor
        seniority_reason = (
            f"Raised to {faculty_floor} — faculty title detected "
            f"({'Professor' if is_full_professor or is_assoc_professor or is_asst_professor else 'Postdoc/PI'})."
        )
