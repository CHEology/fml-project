...
IMPACT_REGEX = re.compile(
    ...
    r"|\b\d+(?:\.\d+)?\s?%"
    r"|\b\d+(?:\.\d+)?\s?(?:x|×)\b"
    # original units (2+ digit requirement relaxed to 1+)
    r"|\b\d+\s?(?:users|customers|clients|requests|qps|ms|hours|days|weeks|months|years|reps|teams|engineers|orders|tickets|sessions|deals|contracts|patients|students)\b"
    # shorthand large numbers: 1M users, 10K requests
    r"|\b\d+(?:\.\d+)?\s?(?:k|m|b)\s?(?:users|customers|clients|requests|records|rows|samples|visits|views|downloads|installs|transactions|events|entries)\b"
    # additional units common in student/research resumes
    r"|\b\d+\s?(?:lines|features|models|papers|citations|awards|grants|courses|datasets|experiments|iterations|releases|people|members|participants|respondents|submissions|repos|commits|prs|bugs|tests|benchmarks|languages|frameworks|papers|publications|projects|companies|startups|schools|universities|countries|cities)\b"
    # performance/size metrics
    r"|\b\d+(?:\.\d+)?\s?(?:gb|tb|mb|kb|fps|rpm|wpm|bps|kwh)\b"
    # GPA pattern
    r"|\b[34]\.\d\s?(?:gpa|grade point)\b"
    # rank/scale signals
    r"|\b(?:top|bottom|ranked?)\s?\d+\b"
    r"|\b#\s?\d+\b"
    # "team of N", "group of N", "cohort of N"
    r"|\b(?:team|group|cohort|class|batch|pool)\s+of\s+\d+\b"
    # Z-scores, p-values, F1, AUC, accuracy
    r"|\b(?:z\s?=\s?|p\s?[<>=]\s?|f1\s?(?:score)?\s?(?:of\s?)?)[\d\.]+\b"
    r"|\b(?:auc|accuracy|precision|recall)\s+(?:of\s+)?(?:0\.\d+|\d+(?:\.\d+)?%)\b"
    r")",
    re.IGNORECASE,
)
...
def assess_quality(
    ...
    if bullet_count == 0:
        ...
        # Multiplier lowered from 220 to 150 — industry standard is 30-40%
        # quantified bullets, not 45%+ that 220 implied.
        impact_score = min(100, int(density * 150))
        # Partial credit floor: any quantified bullet earns at least 20 pts.
        if impact_total >= 1:
            impact_score = max(impact_score, 20)
        # Bonus: reward high absolute counts even on long resumes.
        if impact_total >= 5:
