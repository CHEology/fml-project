"""
Generate synthetic resume/JD evaluation pairs for ResuMatch.

The goal is not naive "write a resume for this job" generation. That tends to
copy JD phrasing and makes retrieval look unrealistically good. This script
creates varied candidate personas, uses only compact skill/title signals from
the source job, avoids copying JD description sentences, and adds a
similar-but-wrong hard negative job for discriminative ranking evaluation.

Usage:
    python scripts/preprocess_data.py
    python scripts/generate_synthetic_resumes.py --jobs data/processed/jobs.parquet \
        --n 100 --out data/eval/synthetic_resumes.parquet

    # If processed jobs are empty because salary filtering removed rows:
    python scripts/generate_synthetic_resumes.py --jobs data/raw/postings.csv \
        --n 100 --out data/eval/synthetic_resumes.parquet
"""
